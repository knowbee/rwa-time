...
exports.getMinute = (date = new Date()) => {
  this.rwaDate(date);
  return isDateValid ? minute : invalidMessage;
};

exports.getFullMinute = (date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${minute !== "01" ? "iminota" : "umunota"} ${minute}`
    : invalidMessage;
};

exports.getSecond = (date = new Date()) => {
  this.rwaDate(date);
  return isDateValid ? second : invalidMessage;
};

exports.getFullSecond = (date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${second !== "01" ? "amasegonda" : "isegonda"} ${second}`
    : invalidMessage;
};

exports.getTime = (sep = ":", date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${hour}${sep}${minute !== "01" ? "iminota" : "umunota"} ${minute}${sep}${
        second !== "01" ? "amasegonda" : "isegonda"
      } ${second}`
    : invalidMessage;
};

exports.getHourMinute = (sep = ":", date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${hour}${sep}${minute !== "01" ? "iminota" : "umunota"} ${minute}`
    : invalidMessage;
};

exports.getFullTime = (sep = " n'", date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${hour}${sep}${minute !== "01" ? "iminota" : "umunota"} ${minute}${sep}${
        second !== "01" ? "amasegonda" : "isegonda"
...
exports.getFullHourMinute = (sep = " n'", date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${hour}${sep}${minute !== "01" ? "iminota" : "umunota"} ${minute}`
    : invalidMessage;
...
exports.getDateTime = (
  ...
  this.rwaDate(date);
  return isDateValid
    ? `${day}${sep1}${monthNumber}${sep1}${year}${sep}${hour}${sep2}${minute}${sep2}${second}`
    : invalidMessage;
};

exports.getFullDateTime = (sep = ", ", sep2 = " n'", date = new Date()) => {
  this.rwaDate(date);
  return isDateValid
    ? `${day}${sep}${month} ${year}${sep}${hour}${sep2}${
        minute !== "01" ? "iminota" : "umunota"
      } ${minute}${sep2}${
        second !== "01" ? "amasegonda" : "isegonda"
      } ${second}`
    : invalidMessage;
};
exports.getFullShortCountryDate = async (sep = ",", date = new Date()) => {
  this.rwaDate(date);
  await this.getShortCountry();
  return isDateValid
    ? `${shortCountry}, ${day} ${month} ${year}`
    : invalidMessage;
};
exports.getFullCityDate = async (sep = ",", date = new Date()) => {
  this.rwaDate(date);
  await this.getCity();
  return isDateValid ? `${city}, ${day} ${month}${sep}${year}` : invalidMessage;
};
exports.getCity = async () => {
  if (city === "") await getUserLocation();
  return city;
};
exports.getRegion = async () => {
  if (region === "") await getUserLocation();
  return region;
};

exports.getCountry = async () => {
  if (country === "") await getUserLocation();
  return country;
};

exports.getTimezone = async () => {
  if (timezone === "") await getUserLocation();
  return timezone;
};

exports.getZipcode = async () => {
  if (zipcode === "") await getUserLocation();
  return zipcode;
};
exports.getShortCountry = async () => {
  if (shortCountry === "") await getUserLocation();
  return shortCountry;
};
  ...
  return new Promise(async (resolve, reject) => {
    let req = await http.get("http://ip-api.com/json", res => {
      ...
      res.on("end", () => {
        ...
        shortRegion = resData.region;
        country = resData.country;
        shortCountry = resData.countryCode;
